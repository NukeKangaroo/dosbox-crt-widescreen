...
static SDL_Surface* GFX_SetupSurfaceScaled(Bit32u sdl_flags, Bit32u bpp) {
	...
	if (sdl.desktop.fullscreen) {
		...
		//ALERT - THIS IS PROBABLY WHERE THE VIEWPORT IS SHRANK
...
Bitu GFX_SetSize(Bitu width, Bitu height, Bitu flags, double scalex, double scaley, GFX_CallBack_t callback) {
	...
	case SCREEN_OPENGL:
	{
		...
		if (sdl.opengl.pixel_data_range) {
			...
			//--------------------ALERT: NEW STUFF
			int time = GetTicks();
			if (crtemu) crtemu_frame(crtemu, (CRTEMU_U32*)crt_frame_to_use, framewidth, frameheight );
			size_t capacity = framewidth * frameheight;
			Bit8u* pixels = (Bit8u*)malloc(capacity);
			crtemu_present(crtemu, (CRTEMU_U64)time, (CRTEMU_U32 const*)pixels, 640 , 400  , framewidth, frameheight, 0xff000000, 0xff282828);
